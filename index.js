...
T.get('search/tweets', {q: '"wake forest" -turnovers -fouls -penalty -crimes -racism -Confederate -turnover -foul -penalty -penalties -fraud -crime commit OR committed exclude:retweets exclude:replies', count: 100, result_type: "recent"}, (err, data,response) => {
  if (!err){
    let tweetArray = new Set();
    let idArray = [];
